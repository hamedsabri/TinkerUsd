    ...
    , m_yLineEdit(new ValueLineEdit<T>(this))
{
    ...
    connect(m_xLineEdit, &ValueLineEdit<T>::valueChanged, this, &AbstractVector2DWidget::onValueEntryChanged);
    connect(m_yLineEdit, &ValueLineEdit<T>::valueChanged, this, &AbstractVector2DWidget::onValueEntryChanged);
}

template <typename T>
void Vector2DEditorWidget<T>::addComponentToLayout(
    QHBoxLayout*      layout,
...
template <typename T> QVector2D Vector2DEditorWidget<T>::getValue() const
{
    return QVector2D(m_xLineEdit->value(), m_yLineEdit->value());
}

template <typename T> void Vector2DEditorWidget<T>::setValue(const QVector2D& vec)
{
    m_xLineEdit->setValue(vec.x());
    m_yLineEdit->setValue(vec.y());
}

template <typename T> void Vector2DEditorWidget<T>::setRange(const QPair<T, T>& range)
{
...
template <typename T> QWidget* Vector2DEditor<T>::createEditor(QWidget* parent) const
{
...
template <typename T> PXR_NS::VtValue Vector2DEditor<T>::toVtValue(const QVariant& value) const
{
...
template <typename T> QVariant Vector2DEditor<T>::fromVtValue(const PXR_NS::VtValue& value) const
{
    if constexpr (std::is_same_v<T, float>)
    {
...
template <typename T> void Vector2DEditor<T>::setEditorData(QWidget* editor, const QVariant& data) const
{
...
template <typename T> QVariant Vector2DEditor<T>::editorData(QWidget* editor) const
{
    AbstractVector2DWidget* vectorEditor = dynamic_cast<AbstractVector2DWidget*>(editor);
    if (vectorEditor)
    {
