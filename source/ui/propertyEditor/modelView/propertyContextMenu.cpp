...
PropertyContextMenu::PropertyContextMenu(PropertyTreeView* treeView, 
    ...
    , m_treeView(treeView)
{
    ...
    // copyPropertyPath / copyPropertyType actions are irrelevant for variants, hence disabling them here.
...
void PropertyContextMenu::copyPropertyValue(const QModelIndex &index)
{
    ...
    if (ArrayEditorBase* arrayEditorBase = dynamic_cast<ArrayEditorBase*>(abstractPropEditor)) {
        ...
        if (value.canConvert<QVector2D>()) {
            ...
            valueStr = QString::fromStdString(std::format("[{:.{}g}, {:.{}g}]", 
                                                          vector.x(), precision, 
                                                          vector.y(), precision));
        } else if (value.canConvert<QVector3D>()) {
            QVector3D vector = value.value<QVector3D>();
            valueStr = QString::fromStdString(std::format("[{:.{}g}, {:.{}g}, {:.{}g}]", 
                                                          vector.x(), precision, 
                                                          vector.y(), precision, 
                                                          vector.z(), precision));
        } else {
